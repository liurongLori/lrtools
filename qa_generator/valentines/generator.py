...
def replace_args(answer):
    ...
    while '${' in answer and count < 5:
        for k, v in args_source_map.items():
            cv = copy.deepcopy(v)
            if k == '${products}' and '${products' in answer:
                vs = random.choice(cv)
                cv.remove(vs)
                answer = answer.replace("${products}", vs)
                vs = random.choice(cv)
                answer = answer.replace("${products1}", vs)
            elif k == '${leading up to}' and '${leading up to' in answer:
                vs = random.choice(cv)
                cv.remove(vs)
                answer = answer.replace("${leading up to}", vs)
                vs = random.choice(cv)
                answer = answer.replace("${leading up to1}", vs)
            elif k in answer:
